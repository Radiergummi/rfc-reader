...
- **The reader's scroll view refuses the trailing safe-area inset** (`ReaderScrollView`), and only the trailing one. The contents panel is a full-height inspector the reader runs underneath, so its width arrives as a safe-area inset that a scroll view turns into content insets the text view tracks — that re-wraps the text and rebuilds the document, which is what the panel exists to avoid. Zeroing the insets outright instead puts the first lines behind the toolbar. `ignoresSafeArea` does not reach the clip view from any level above it.
