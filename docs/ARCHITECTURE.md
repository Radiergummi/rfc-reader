...
- **Anchors are stable strings**, not indices. XML documents use the author's `anchor` or the RFC Editor's `pn` part number; text documents synthesise `section-4.2`, `appendix-A`, `name-security-considerations`. A bibliography entry keeps its label when the label is an XML name (`RFC2119`, `MIP-OPTIM`); one that is not (`[1]`, `[RFC 2119]`) is anchored under the document it cites (`RFC791`) or else `ref-` and the label spelled as a name, and keeps the label as `displayAnchor` either way. Deep links, the table of contents and reading positions all key off these.
