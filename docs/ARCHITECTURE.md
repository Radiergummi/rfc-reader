...
- **A heading is inlines, not a string.** *Decided September 2026.* Headings cite documents — "Changes from RFC 3066", "Differences from RFC 793", some 3,500 of them across the corpus — and a `String` title could not carry the link however well the linker worked. `Section.title` is `[Inline]`; `titleText` and `displayTitle` flatten it for the outline, anchors and search, and `displayTitleInlines` is what the reader draws. The number is never part of the words: it lives in `number` and the reader composes `4.2. ` around whatever the heading says, so the linker cannot mistake a heading's own number for a section reference.
