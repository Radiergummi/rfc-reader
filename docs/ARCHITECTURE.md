...
- **Anchors are stable strings**, not indices. XML documents use the author's `anchor` or the RFC Editor's `pn` part number; text documents synthesise `section-4.2`, `appendix-A`, `name-security-considerations`. A bibliography entry keeps its label when the label is an XML name (`RFC2119`, `MIP-OPTIM`); one that is not (`[1]`, `[RFC 2119]`) is anchored under the document it cites (`RFC791`) or else `ref-` and the label spelled as a name, and keeps the label as `displayAnchor` either way. Entry anchors are settled before the prose is linked: a repeat takes the first `-2`, `-3` no other entry or section holds, and a label listed twice is cited as its first entry. Deep links, the table of contents and reading positions all key off these.
...
- **Abbreviations are collected at parse time too** (`Abbreviations`, issue #67), from the document's own text: `Transport Layer Security (TLS)` at first use, and `TLS: Transport Layer Security` in a glossary definition list, where the whole opening phrase has to be the expansion. Both parsers set `RFCDocument.abbreviations` as their last step, keyed by the short form as written, and the first expansion wins. The long form is found with Schwartz and Hearst's algorithm, chosen for precision: every letter of the short form has to match, in order, and its first has to start a word, or there is no expansion. A wrong expansion shown confidently is worse than none. Bibliography entries and verbatim blocks are not read. The RPC's abbreviation list, the fallback for what a document never expands, waits on the licensing question in `DATA_PIPELINE.md`.
- **An author's contact details are data, not body text** (#19, #113). `Author.contact` holds what RFCXML's `<author>` publishes: `<organization>` and an `<address>` with its `<postal>` (structured, or the author's own `<postalLine>`s), phone, fax, emails and URI. It is nil for every author a legacy header or the RFC index names, since nothing is looked up or inferred. Prep's "Authors' Addresses" section is built from the same elements. It was read as unknown containers, an aside for each level of `<author><address><postal>`, which the schema rejects on a round trip. It now reads as one paragraph per author, a detail per line, with email and web addresses as links. The parser's fallback for an unknown container also no longer nests an aside in an aside. A contact card, and an Info tab for the inspector, are meant to show these instead (#19, #25).
...
**RFCXML v3** (`RFCXMLParser`): the whole file is loaded into a tiny DOM (`XMLTree`) and walked. Documents are at most a couple of megabytes; RFC 9110 (1.2 MB, 305 sections, 1,376 paragraphs) parses in about 50 ms in a release build. Whitespace inside `<t>` is collapsed as HTML would; artwork and source code are kept byte-for-byte. A `<u>` element is spelled out as its `format` asks (by default `"ש" (HEBREW LETTER SHIN, U+05E9)`), with the literal as code, because the vocabulary requires the code point in every rendering and the bare character depends on a font (`UnicodeNotation`, issue #63). `<artset>` picks the ASCII alternative until there is an SVG renderer.

**Legacy text** (`LegacyTextParser`), in order:
1. *Depaginate*: drop form feeds, `[Page N]` footers and the running header that follows a page break; remember where breaks were. Then drop the lines that recur at the page edges however they are worded (RFC 793's `Transmission Control Protocol`) — but only a line set off from the body by a blank and seen at the edges more often than inside the pages, because the body recurs too and a MIB's `STATUS current` lands at a page edge by chance. A line on half the pages names the document — half the pages of its parity, where headers alternate between facing pages (RFC 821). A section running header must head every page, or every other, of three or more, as part of the block that opens the page: the body starts below the blank lines after a page's header, and RFC 6208's `Additional information:` at the head of five pages is the body's. A line wrongly kept costs a stray heading; one wrongly dropped is lost text, so the rule errs towards keeping. `corpus/report.json` records each document's dropped lines as `furniture`, because the block counts cannot show that loss.
2. *Front matter*: the two-column header block (`Request for Comments: 1149`, `Obsoletes:`, `Category:`, authors, date) up to the first blank line, then the centred title. It runs on to the first run that is the body's — one holding a column-0 heading, or a paragraph before that — and ends at that run's start; with no heading at all, it ends after the title. Front matter has nowhere to put a paragraph, so running on to the first heading lost everything above it (#60: RFC 809's 1,951 lines up to its appendix).
3. *Headings*: any non-indented line. `1.2.` → numbered; `Appendix B.` / `B.1.` → appendix; anything else unnumbered. Depth from the number; sections nest with a stack. `1:` and `2.4.12:` are numbered too, where the document numbers its headings that way (#71: RFC 2078, 2743 and 2130 all through, five others for a few). The same shape is a field label, a line-numbered listing or a second numbering elsewhere, so it counts only where two things hold across the document: no colon number is also the number of a `1.` heading, since a document numbers each section once (RFC 705 lists `1.  BEGIN Command` and describes it again under `1:  BEGIN`); and every colon number follows from a heading number — the one before it at its level, or one it is under — because a lone `11:` with no `10` is a time (RFC 526), not a numbering.
...
6. *Boilerplate* ("Status of This Memo", "Table of Contents", copyright) is dropped from the model but stays in the "original text" view, which is `stripPagination(_:)` over the same file. A dropped section, or the abstract, ends at the next heading, so where the body's headings are a shape the parser does not know it would take the body with it (#60, RFC 908: 60 pages inside `Status of This Memo`). One that runs past 20 blocks — no boilerplate in the corpus is more than 16 — keeps only its leading paragraphs, or contents entries, and the rest is body.
...
*Decided September 2026 (issues #28, #29).* `ReferencePreview` is presented by a 0.5 s hover dwell on macOS — an `NSTrackingArea` owned by the coordinator, then an `NSPopover` anchored to the reference's whole extent — and by `textView(_:menuConfigurationFor:defaultMenu:)` on iOS, with the default menu alongside. Both look the reference up with `NSAttributedString.reference(at:)` in `RFCReaderKit`, which takes the attribute's `longestEffectiveRange`: a chip is three storage runs, so the storage run is a third of it. A force click on a reference shows the same card at once — the "preview on hard press" of VISION.md's Tier 1 — through `ReaderTextView`'s `quickLook(with:)`, which takes the event only when a reference is under it and otherwise calls `super`, meaning to leave Look Up working on every other word. **Neither half is verified on Force Touch hardware yet**: `NSTextView` runs a private immediate-action recognizer that may claim the gesture before `quickLook(with:)` is reached, and if it does, the fallback is to recognise the press in `pressureChange(with:)` at stage 2 instead. The force click's own mouse-up may also arrive as a click on the link; `clickedOnLink` swallows one click on the reference the force click previewed, and the next mouse-down forgets it. Matching `eventNumber` instead was tried and is a crash: it raises on any event that is not a mouse event, which Look Up from a gesture or the keyboard is. A mouse-down, a context menu or a new document cancels a dwell in progress: the timer runs in `.default` mode, so a click's or a drag's tracking loop would only delay it until the button came up, and then open the card wherever the drag ended. A force click on a reference that has no card is Look Up's, like any other word. Scrolling closes the card and, once it has stopped for the dwell, hit-tests under the resting pointer — once, not per scroll tick. The scroll that following a link causes does not: until the pointer moves from where it clicked, nothing it lands on previews. A card that closes itself (Esc, a click elsewhere, the app going inactive) ends its hover through the popover's delegate, so the same reference can preview again without the pointer leaving it first, and Look Up carrying another window's event, a menu's, is left to Look Up. The tracking area is `.activeInActiveApp`, not `.activeInKeyWindow`: a popover's window can become key (`_NSPopoverWindow.canBecomeKey` is true), and with it the moves and the exit that close the card would stop. A reference to another document shows its title, status and abstract; one within the document shows its section's heading, which the builder records on the section's `AnchorIndex` entry — the heading is what makes an entry a section, so the two cannot disagree — and one to a figure or a table shows nothing, having nothing to add to its own words.

The hover machinery existed from the start and never fired, which is worth knowing about AppKit: a tracking area sends its owner `mouseMoved:`, and on a class that is not an `NSResponder` Swift names `@objc func mouseMoved(with:)` `mouseMovedWith:`. AppKit skips an owner that does not respond, so the area was installed and nothing arrived. Spell the selector out, `@objc(mouseMoved:)`. The raw `rfc://` tooltip that appeared instead is `NSTextView.displaysLinkToolTips`, on by default, which the reader turns off. An external link's destination is still worth reading before it is followed, so `DocumentTextBuilder` gives an external link an explicit `.toolTip` of its URL, and a reference none. That is correct by construction rather than by a delegate filtering what AppKit proposes. It is also observed working: in a TextKit 2 `NSTextView` with `displaysLinkToolTips` off, a mouse move over a `.toolTip` run reaches `textView(_:willDisplayToolTip:forCharacterAt:)` with that tooltip, and one over a link reaches nothing; with it on, the link proposes its URL through the same call.
...
*Found September 2026, all three by instrumenting a running build rather than by reading.*
...
- A block folded per RFC 8792 is shown unfolded, without its header, when every unfolded line fits the column at full size, and otherwise exactly as the page printed it, header and backslashes included (`DocumentTextBuilder.displayedText`, issue #64). A block that does not fit is not scaled down to fit unfolded, because its unfolded lines can be far longer than the widths fit-to-measure scaling was measured against. "Copy Figure" always copies it unfolded (`FoldedLines` in RFCKit, `FigureCopy` in RFCReaderKit). A plain selection copies whatever is on screen, so across a block still shown folded it copies the folded lines.
