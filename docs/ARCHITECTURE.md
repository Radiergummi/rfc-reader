...
- A block folded per RFC 8792 is shown as the page printed it, header and backslashes included. Only "Copy Figure" unfolds it (`FoldedLines` in RFCKit, `FigureCopy` in RFCReaderKit), because copied code has to work and a displayed block has to fit. Showing it unfolded where the column is wide enough would need a fallback to the folded text or to fit-to-measure scaling for a block that still does not fit (issue #64). A plain selection across a folded block still copies the folded lines.
