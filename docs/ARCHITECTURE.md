...
The toolbar is in the **expanded** style, which gives the title and subtitle a row of their own. In the unified style a window that shows a title draws it as a block at the start of the document's toolbar section, and that block expands to fill: Back and Forward were pushed from 204 pt out to 1199 on a 1500 pt window — with and without a subtitle — landing hard against the document's own actions. A leading `NSTitlebarAccessoryViewController` does sit before the title, but it is laid out over the sidebar and pushes the sidebar's own toggle into the toolbar's overflow menu. Expanded is the one arrangement that has all four: title and subtitle, Back and Forward at the leading edge of the content pane, the sidebar's toggle inside the sidebar, and the panel's toggle in the window's trailing corner.
