...
**RFCXML v3** (`RFCXMLParser`): the whole file is loaded into a tiny DOM (`XMLTree`) and walked. Documents are at most a couple of megabytes; RFC 9110 (1.2 MB, 305 sections, 1,376 paragraphs) parses in about 50 ms in a release build. Whitespace inside `<t>` is collapsed as HTML would; artwork and source code are kept byte-for-byte. `<artset>` picks the ASCII alternative until there is an SVG renderer. `<u>`, which marks non-ASCII text a protocol depends on, is expanded as its `format` asks (`"ש" (HEBREW LETTER SHIN, U+05E9)` by default) rather than read as the bare character, which a font may not have and a reader may not tell from a lookalike; the literal stays `.code`, so it is neither linked nor collapsed.
