...
struct ChipLineGeometryTests {
    ...
    @Test func aChipOnALaterLineDrawsBehindItsOwnText() throws {
        ...
        for chip in onLaterLine {
            #expect(chip.rect.minX > FragmentGeometry.chipPadding, "a chip on a later line must not draw at the line's left edge")
        }
    }
    ...
    @Test func hitTestingResolvesInsideTheChipItPointsAt() throws {
        ...
        let resolved = try #require(FragmentGeometry.characterOffset(
            in: fixture.lines,
            fragmentStart: fixture.fragmentStart,
            at: point
        ))
        #expect(resolved >= piece.location && resolved < NSMaxRange(piece), "must resolve inside the chip under the pointer, not past it")
    }
