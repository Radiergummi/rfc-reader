...
struct DecorationRunTests {
    ...
    @Test func aStackedTablesCellsAreOneDecorationRun() throws {
        let table = RFCKit.Table(
            ...
            rows: [
                [[Inline.text("alpha")], [Inline.text(String(repeating: "a long prose description ", count: 4))]],
                [[Inline.text("beta")], [Inline.text(String(repeating: "another long description ", count: 4))]],
            ],
            anchor: "table-1"
        )
    ...
    func noBlockIsSplitIntoTwoRuns(fixture: String) throws {
        ...
        built.text.enumerateAttribute(.rfcDecoration, in: NSRange(location: 0, length: built.text.length)) { value, range, _ in
            ...
            let gap = NSRange(location: NSMaxRange(previous.range), length: range.location - NSMaxRange(previous.range))
