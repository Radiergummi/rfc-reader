...
struct AdjacentCardTests {
    ...
    layout.enumerateTextLayoutFragments(
      ...
      guard
        let span = FragmentGeometry.decorationSpan(
          in: text, fragment: NSRange(location: start, length: end - start))
      else {
        return true
      }
      let frame = fragment.layoutFragmentFrame
      let placement = FragmentGeometry.Placement(
        origin: frame.origin, frame: frame, containerWidth: style.measure, indent: span.indent)
  ...
  @Test(arguments: [
    ...
    [
      Block.preformatted(Preformatted(kind: .artwork, text: "AAAA")),
      .preformatted(Preformatted(kind: .sourceCode, text: "BBBB", type: "abnf")),
    ],
  ...
  func adjacentCardsMeetWithAGap(blocks: [Block]) throws {
    ...
    let lower = drawn[1]
    #expect(
      NSMaxRange(upper.run) == lower.run.location,
      "the blocks must be adjacent for this to test anything")
    #expect(
      upper.rect.maxY < lower.rect.minY,
      "the cards overlap: \(upper.rect.maxY) past \(lower.rect.minY)")
  }
