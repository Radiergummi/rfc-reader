...
struct FigureCopyTests {
    ...
    @Test func aMenuOnAFigureFindsIt() throws {
        let text = built().text
        let figure = try #require(FigureCopy.figure(at: try Fixtures.offset(of: "| A |", in: text), in: text))
    ...
    @Test func theLanguageLabelBelongsToItsFigure() throws {
        let text = built().text
        let figure = try #require(FigureCopy.figure(at: try Fixtures.offset(of: "JSON", in: text), in: text))
    ...
    @Test func aSelectionMeansAFigureOnlyWhenItTouchesExactlyOne() throws {
        ...
        #expect(FigureCopy.figure(in: NSRange(location: diagram, length: 0), of: text)?.text == Self.diagram.text)
    }
    ...
    @Test func aFoldedFigureIsCopiedUnfolded() throws {
        let text = built().text
        #expect(text.string.contains("NOTE: '\\' line wrapping"), "the reader still shows the block as published")
        let figure = try #require(FigureCopy.figure(at: try Fixtures.offset(of: "a long", in: text), in: text))
