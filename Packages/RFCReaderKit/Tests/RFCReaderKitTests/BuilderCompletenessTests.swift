...
struct BuilderCompletenessTests {
    ...
    func nothingBecomesAnAttachment(fixture: String) throws {
        ...
        while searchStart < text.length {
            let found = text.range(of: "\u{FFFC}", range: NSRange(location: searchStart, length: text.length - searchStart))
    ...
    @Test func blockQuotesAndAsidesAreIndentedTextWithADecoration() throws {
        let document = Fixtures.document(.blockQuote([.paragraph(Paragraph(text: "quoted"))]),
        ...
        for (needle, expected) in [("quoted", RFCDecoration.blockQuote), ("noted", RFCDecoration.aside)] {
            ...
            #expect(RFCDecoration(attributeValue: built.text.attribute(.rfcDecoration, at: offset, effectiveRange: nil)) == expected)
    ...
    @Test func aNestedAsideInsideABlockQuoteKeepsItsOwnDecoration() throws {
        let document = Fixtures.document(.blockQuote([
