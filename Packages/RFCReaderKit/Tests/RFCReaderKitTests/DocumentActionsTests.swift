...
struct DocumentActionsTests {
    ...
    @Test func theDocumentNamesItWhenTheIndexDoesNot() {
        let title = DocumentActions.bookmarkTitle(metadata: nil, documentTitle: "HTTP Semantics", id: id)
        #expect(title == "HTTP Semantics")
    }
    ...
    @Test func aCitationCarriesTheSectionBeingRead() {
        let cited = DocumentActions.citation(metadata(title: "HTTP Semantics"), section: "4.2", style: .full)
    ...
    @Test func bibtexCitesTheWholeDocumentEvenWhileASectionIsBeingRead() {
        let withSection = DocumentActions.citation(metadata(title: "HTTP Semantics"), section: "4.2", style: .bibtex)
        let without = DocumentActions.citation(metadata(title: "HTTP Semantics"), section: nil, style: .bibtex)
        #expect(withSection == without)
    }

    @Test func everyOtherStyleKeepsTheSection() {
        for style in CitationStyle.allCases where style != .bibtex {
            let withSection = DocumentActions.citation(metadata(title: "HTTP Semantics"), section: "4.2", style: style)
            let without = DocumentActions.citation(metadata(title: "HTTP Semantics"), section: nil, style: style)
