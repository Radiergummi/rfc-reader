...
struct DecorationGeometryTests {
    ...
    private func rect(_ fragment: CGRect, indent: CGFloat = 0, capTop: Bool = false, capBottom: Bool = false) -> CGRect {
        placement(fragment, indent: indent).decorationRect(padding: padding, capTop: capTop, capBottom: capBottom)
    }
    ...
    @Test func theBandIndentIsTheShallowestInTheRun() {
        ...
        text.append(NSAttributedString(string: "China Mobile\n", attributes: [.paragraphStyle: para(20)]))
    ...
    @Test func theIndentComesFromTheParagraphStyle() {
        ...
        #expect(FragmentGeometry.indent(in: NSAttributedString(string: "plain"), over: NSRange(location: 0, length: 1)) == 0)
        #expect(FragmentGeometry.indent(in: text, over: NSRange(location: 99, length: 1)) == 0, "an out-of-range offset must not trap")
    }
}
