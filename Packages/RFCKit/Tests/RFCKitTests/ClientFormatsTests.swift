...
struct ClientFormatsTests {
    @Test func perDocumentJSON() throws {
        let record = try JSONDecoder().decode(RFCEditorMetadataRecord.self, from: try Fixtures.data("rfc9110.json"))
    ...
    @Test func endpoints() {
        ...
        #expect(RFCEditorEndpoints.document(id, format: .xml).absoluteString == "https://www.rfc-editor.org/rfc/rfc9110.xml")
        #expect(RFCEditorEndpoints.metadata(id).absoluteString == "https://www.rfc-editor.org/rfc/rfc9110.json")
        #expect(RFCEditorEndpoints.infoPage(id).absoluteString == "https://www.rfc-editor.org/info/rfc9110")
        #expect(RFCEditorEndpoints.datatracker(id, section: "4.2").absoluteString == "https://datatracker.ietf.org/doc/html/rfc9110#section-4.2")
    }
