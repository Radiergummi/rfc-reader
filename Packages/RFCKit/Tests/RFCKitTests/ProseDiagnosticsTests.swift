...
struct ProseDiagnosticsTests {
    ...
    @Test func oneArrowRejectsAnOtherwiseOrdinaryParagraph() {
        ...
        #expect(diagnosis.artworkMatches == 1, "a single incidental match, against every other signal agreeing")
    ...
    @Test func artworkMatchesAreCountedNotJustDetected() {
        ...
        #expect(diagnosis.artworkMatches > 1, "real artwork matches repeatedly; incidental prose matches once")
        #expect(!diagnosis.isNearMiss)
    }
    ...
    func everyClassifiedBlockIsAlsoDiagnosed(fixture: String) throws {
        ...
        #expect(classified > 0)
        #expect(diagnosed.count >= classified, "classification never invents a block the segmentation did not produce")
    }
    ...
    @Test func listItemsAreMarkedAsNeverReachingTheProseTest() throws {
        ...
        #expect(!outdented.isEmpty, "the hanging marker is exactly what makes these look like rejected prose")
        #expect(
