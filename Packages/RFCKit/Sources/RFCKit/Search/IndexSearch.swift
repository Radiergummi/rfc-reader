...
public struct IndexSearch: Sendable {
    ...
    private func score(_ entry: Entry, rfc: RFCMetadata, terms: [SearchText], loweredQuery: SearchText) -> Int? {
        var total = 0
        for term in terms {
            var best = 0
            ...
            else if entry.title.contains(term) { best = max(best, 45) }
            if entry.keywords.contains(term) { best = max(best, 50) }
            else if entry.keywords.contains(where: { $0.contains(term) }) { best = max(best, 30) }
